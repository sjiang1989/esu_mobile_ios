	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if (response.messages != null && response.messages.length > 0) {
			...
			for (Message message : response.messages) {
				...
				// text color
				int statusColor = VersionSupportUtils.getColorHelper(getContext(), R.color.status_important_text_color);
			...
			for (RegisterSection section: response.successes) {
				...
				if (section.messages != null) {
					for (Message message : section.messages) {
						...
						messageView.setText(message.message);
						// text color
						int statusColor = VersionSupportUtils.getColorHelper(getContext(), R.color.status_success_text_color);
						messageView.setTextColor(statusColor);
			...
			for (RegisterSection section: response.failures) {
				...
				if (section.messages != null) {
					for (Message message : section.messages) {
						...
						messageView.setText(message.message);
						// text color
						int statusColor = VersionSupportUtils.getColorHelper(getContext(), R.color.status_error_text_color);
						messageView.setTextColor(statusColor);
						sectionRow.addView(messageView);
					}
				}
